    ...
    const renderCard = ({ item }) => {
        return (
            <View style={styles.itemContainer}>
                ...
                <Button buttonStyle={styles.button} title="ADD" />
            </View>
        );
    };
    if (!props.category) {
        return <FlatList data={products} renderItem={renderCard} keyExtractor={item => item.id.toString()} />;
    }
    return <FlatList data={products.filter(item => item.category === props.category)} renderItem={renderCard} keyExtractor={item => item.id.toString()} />;
}
